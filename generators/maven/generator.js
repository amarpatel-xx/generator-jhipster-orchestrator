    ...
    return this.asComposingTaskGroup({
      async composingTemplateTask() {
        if (['sql'].includes(this.jhipsterConfigWithDefaults.databaseType)) {
          // Delegate the client sub-generator to the maven generator.
          await this.composeWithJHipster('jhipster-orchestrator:maven-orchestrator');
        }
      },
    });
  }
    ...
    return this.asWritingTaskGroup({
      async writingTemplateTask({ application }) {
        await this.writeFiles({
          sections: { files: [{ templates: ['template-file-maven'] }] },
          context: application,
        });
      },
    });
