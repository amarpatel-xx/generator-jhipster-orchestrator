export const angularSaathratriUtils = {
  ...
  getTypescriptType(field) {
    ...
    if (field.fieldIsEnum) {
      ...
      ["Integer", "Long", "Float", "Double", "BigDecimal"].includes(fieldType)
    ) {
      tsType = "number";
   ...
   */
  getCompositePrimaryKeyClientModelGenerateEntityClientFieldDefaultValues(
    fields,
  ) {
    ...
    fields.forEach((field) => {
      ...
      if (field.isCompositePrimaryKeyClientField && fieldType === "Boolean") {
        defaultVariablesValues[fieldName] =
          `this.${fieldName} = this.${fieldName} ?? false;`;
      }
    });
  ...
  getCompositePrimaryKeyClientServiceUrl(entityInstanceName, primaryKey) {
    return primaryKey.ids
      .map((pk) =>
        pk.fieldTypeTimed ||
        pk.fieldContainsUtc ||
        pk.fieldTypeTimedSaathratri ||
        pk.fieldContainsUtcSaathratri
  ...
  getCompositePrimaryKeyClientUpdateComponentUndefinedCheck(
    entityInstanceName,
    primaryKey,
  ) {
    return primaryKey.ids
      .map(
        (pk) =>
          `${entityInstanceName}.${primaryKey.name}?.${pk.fieldName} !== undefined`,
      )
      .join(" && \n");
  },

  getCompositePrimaryKeyClientServiceModelPrimaryKeyVariableDeclarationStatements(
    primaryKey,
    fields,
    variablesWithTypes,
  ) {
    ...
    fields.forEach((field) => {
      if (primaryKeyFieldNames.includes(field.fieldName)) {
        if (field.options) {
          if (
            field.fieldTypeLocalDateSaathratri ||
            field.fieldTypeTimedSaathratri
          ) {
            variableDeclarations.push(`${field.fieldName}: dayjs.Dayjs | null`);
          } else {
            variableDeclarations.push(
              `${field.fieldName}: ${this.getTypescriptType(field)} | null`,
            );
          }
        }
      }
  ...
  getCompositePrimaryKeyClientServiceModelNonPrimaryKeyVariableDeclarationStatements(
    primaryKey,
    fields,
    variablesWithTypes,
  ) {
      ...
      if (!primaryKeyFieldNames.includes(field.fieldName)) {
        if (field.options) {
          if (field.fieldTypeMapDayjsSaathratri) {
            ...
            variableDeclarations.push(
              `${field.fieldName}?: dayjs.Dayjs | null`,
            );
